...
GEBool GE_STDCALL SayEx_sAIOutputInstr(gSAIOutputInstr_Args *a_pArgs, gCScriptProcessingUnit *a_pSPU,
                                       GEBool a_bUnk /* , gSArgsFor__AI_Say_Ex * a_pSayArgs@<esi> */)
{
...
bCString const *GE_STDCALL gCScriptProcessingUnit_CalcSubtitleDisplayTime(gCScriptProcessingUnit *a_pSPU,
                                                                          gSAIOutputInstr_Args_Ext *a_pArgs)
{
...
extern "C" __declspec(dllexport) gSScriptInit const *GE_STDCALL ScriptInit(void)
{
    ...
    Hook_SayEx_sAIOutputInstr.Hook(RVA_ScriptGame(0x49472), &SayEx_sAIOutputInstr, mCBaseHook::mEHookType_Mixed,
                                   mERegisterType_Esi);
    ...
    Hook_CalcSubtitleDisplayTime.Prepare(RVA_Game(0x1694E8), &gCScriptProcessingUnit_CalcSubtitleDisplayTime)
        .AddRegArg(mERegisterType_Esi) // gCScriptProcessingUnit
        .AddRegArg(mERegisterType_Edi) // gSAIOutputInstr_Args_Ext
        .ReplaceSize(0x13)
        .Hook();
