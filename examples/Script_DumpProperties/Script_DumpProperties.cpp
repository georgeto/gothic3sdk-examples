...
extern "C" __declspec(dllexport) gSScriptInit const *GE_STDCALL ScriptInit(void)
{
    ...
    for (bTPtrMap<bCString, bCPropertyObjectSingleton::bCPOTypeListItem *>::bCConstIterator Iter =
             TypeSystem.BeginIterator();
         Iter != TypeSystem.EndIterator(); Iter++)
    {
        bCPropertyObjectTypeBase const *pPropertyObjectType = (*Iter)->GetObjectType();

        value::object Type;
        Type["key"] = value(Iter.GetKey());
        Type["name"] = value(pPropertyObjectType->GetClassName());
        ...
        for (bTArrayBase<bCPropertyTypeBase *>::bCConstIterator PropertyIter = pPropertyObjectType->BeginIterator();
             PropertyIter != pPropertyObjectType->EndIterator(); PropertyIter++)
        {
