...
{
    HookOnExecute.Hook(PROC_Engine("?OnExecute@eCConsole@@MAE_NABVbCString@@AAV2@@Z"), &OnExecuteHook,
                       mCFunctionHook::mEHookType_ThisCall);
    HookOnComplete.Hook(PROC_Engine("?OnComplete@eCConsole@@MAE_NAAVbCString@@@Z"), &OnCompleteHook,
                        mCFunctionHook::mEHookType_ThisCall);
    HookPrintChoices.Hook(PROC_Engine("?PrintChoices@eCConsole@@UAEXABVbCString@@@Z"), &PrintChoicesHook,
                          mCFunctionHook::mEHookType_ThisCall);
    HookPrintOutput.Hook(PROC_Engine("?PrintOutput@eCConsole@@QAEXABVbCString@@ABVbCByteAlphaColor@@@Z"),
                         &PrintOutputHook, mCFunctionHook::mEHookType_ThisCall);
...
GEBool mCCommandHandler::OnExecuteHook(bCString const &a_rCommand, bCString &a_rStrResult)
{
    ...
    // a_rCommand.GetText(), a_rStrResult.GetText(), keyboard.KeyPressed((eCInpShared::eEKeyboardStateOffset)42),
    // keyboard.KeyPressed((eCInpShared::eEKeyboardStateOffset)29),
    // keyboard.KeyPressed((eCInpShared::eEKeyboardStateOffset)56),
    // keyboard.KeyPressed((eCInpShared::eEKeyboardStateOffset)15));
...
GEBool mCCommandHandler::OnCompleteHook(bCString &a_rCommand)
{
    ...
    // a_rCommand.GetText(), keyboard.KeyPressed((eCInpShared::eEKeyboardStateOffset)42),
    // keyboard.KeyPressed((eCInpShared::eEKeyboardStateOffset)29),
    // keyboard.KeyPressed((eCInpShared::eEKeyboardStateOffset)56),
    // keyboard.KeyPressed((eCInpShared::eEKeyboardStateOffset)15));
...
GEBool mCCommandHandler::RegisterCommand(bCString const &a_strName, bCString const &a_strDescription)
{
    ...
    eCApplication::GetInstance().GetConsole().AddCommand(CommandDesc.m_strName, CommandDesc.m_strDescription, 0,
                                                         0xFFFFFFFFFFFFFFFF, g_strDefault);
...
mCCommandHandler::mSSubCommandDesc *mCCommandHandler::GetSubCommand(mSCommandDesc &a_CommandDesc,
                                                                    bCString const &a_strName)
{
...
mCCommandHandler::mEResult mCCommandHandler::OnExecute(bCString const &a_strCommand, bCString &a_strResult)
{
    ...
    if (iParamCount == 1)
        return a_strResult = "Please specify a command (use \"" + pCommand->m_strName
    ...
    if (pSubCommand->m_iMinArgs > iParamCount - 2
        || (pSubCommand->m_iMaxArgs != -1 && pSubCommand->m_iMaxArgs < iParamCount - 2))
        return a_strResult = "Wrong argument count. You can use \"" + pCommand->m_strName + " " + pSubCommand->m_strName
                           + " help\" to learn more about the command's usage.",
               Failed;
...
mCCommandHandler::mEResult mCCommandHandler::OnComplete(bCString &a_strCommand)
{
    ...
    if (pSubCommand->m_pfuncAutoComplete != 0)
        return a_strCommand = JoinString(arrParams, 0, " ", 2) + " "
                            + (*pSubCommand->m_pfuncAutoComplete)(CopyRange(arrParams, 2)),
               Successful;
...
mCCommandHandler::mEResult mCCommandHandler::PrintChoices(bCString const &a_strCommand)
{
    ...
    bCString strHelpText =
        bHelpType ? GetCommandHelpText(pCommand->m_strName) : GetSubCommandHelpText(*pCommand, arrParams[1]);
...
GEInt NextCursorPosition(eCEditCtrl &a_EditCtrl, GEInt a_iDirection)
{
    ...
    else
    {
        ...
        GEInt iNextWordBoundary =
            strText.FindOneOf(s_WordChars, iCursorPos + 1 < iTextLength ? iCursorPos + 1 : iTextLength);
...
GEInt eCConsoleDlg_OnKeyDown(/* eCConsoleDlg & This, */ eCInpShared::eEKeyboardStateOffset a_Key,
                             eCInpShared::eSInputMessage const &a_Message)
{
    ...
    if (bCtrlPressed
        && (a_Key == eCInpShared::eEKeyboardStateOffset_Z || a_Key == eCInpShared::eEKeyboardStateOffset_Y))
...
void eCEditCtrl_OnKeyDown(eCEditCtrl &a_EditCtrl, eCInpShared::eEKeyboardStateOffset a_Key,
                          eCInpShared::eSInputMessage const &a_Message)
{
...
{
    Hook_eCConsoleDlg_OnKeyDown.Hook(
        PROC_Engine("?OnKeyDown@eCConsoleDlg@@UAEHW4eEKeyboardStateOffset@eCInpShared@@ABUeSInputMessage@3@@Z"),
        eCConsoleDlg_OnKeyDown, mCBaseHook::mEHookType_ThisCall);

    Hook_eCEditCtrl_OnKeyDown.Prepare(RVA_Engine(0xCCCF5), eCEditCtrl_OnKeyDown)
        .AddRegArg(mERegisterType_Esi) // EditCtrl
        .AddRegArg(mERegisterType_Edi) // Key
        .AddRegArg(mERegisterType_Ebp) // Message
        .InsertCall()
