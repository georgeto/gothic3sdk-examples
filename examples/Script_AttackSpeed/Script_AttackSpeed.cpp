...
void LoadSettings()
{
    ...
    {
        g_fAttackSpeedMultiplicator =
            config.GetFloat(bCString("Game"), bCString("Game.AttackSpeedMultiplicator"), g_fAttackSpeedMultiplicator);
    }
}
...
extern "C" __declspec(dllexport) gSScriptInit const *GE_STDCALL ScriptInit(void)
{
    ...
    LoadSettings();
    Hook_ApplyAttackSpeedMultiplicator.Prepare(RVA_ScriptGame(0x4D5B), &ApplyAttackSpeedMultiplicator)
        .InsertCall()
        .AddStackArg(0x10)
        .Hook();
