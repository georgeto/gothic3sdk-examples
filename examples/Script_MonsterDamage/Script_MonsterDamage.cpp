...
void LoadSettings()
{
    ...
    if (config.ReadFile(bCString("monsterdamage.ini")))
    {
        fMonsterDamageMultiplicator =
            config.GetFloat(bCString("Game"), bCString("Game.MonsterDamageMultiplicator"), GEFalse);
    }
}
...
extern "C" __declspec(dllexport) gSScriptInit const *GE_STDCALL ScriptInit(void)
{
    ...
    Hook_CalculateMonsterDamage
        .Prepare(RVA_ScriptGame(0x2C06D), &CalculateMonsterDamage, mCBaseHook::mEHookType_Mixed, mERegisterType_Esi)
        .ReplaceSize(0x11)
        .Hook();
