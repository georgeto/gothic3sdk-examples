...
gEDamageType GE_STDCALL AI_HoldInventoryItems_Undraw_Effect(PSDamage const &ItemDamage,
                                                            gSArgsFor__AI_HoldInventoryItems const &Args)
{
...
gEDamageType GE_STDCALL AI_HoldInventoryItems_Draw_Effect(PSDamage const &ItemDamage,
                                                          gSArgsFor__AI_HoldInventoryItems const &Args)
{
    Entity Item(ItemDamage.IsValid() ? ItemDamage.m_pEngineEntityPropertySet->GetEntity() : nullptr);
...
void ApplyHooks()
{
    ...
    Hook_GetAniName_GetActorName.Prepare(RVA_Game(0x16F8B9), &GetAniName_GetActorName)
        .AddRegArg(mERegisterType_Ebx) // eCEntity const * Actor
        .AddRegArg(mERegisterType_Esi) // bCString & ActorName
        .InsertCall()
        .Hook();
    ...
    Hook_AI_HoldInventoryItems_Undraw_Effect_Left.Prepare(RVA_ScriptGame(0x43DB8), &AI_HoldInventoryItems_Undraw_Effect)
        .AddThisArg()
        .AddRegArg(mERegisterType_Esi)
        .Hook();

    static mCCallHook Hook_AI_HoldInventoryItems_Draw_Effect_Right;
    Hook_AI_HoldInventoryItems_Draw_Effect_Right.Prepare(RVA_ScriptGame(0x448CD), &AI_HoldInventoryItems_Draw_Effect)
        .AddThisArg()
        .AddRegArg(mERegisterType_Esi)
        .Hook();

    static mCCallHook Hook_AI_HoldInventoryItems_Draw_Effect_Left;
    Hook_AI_HoldInventoryItems_Draw_Effect_Left.Prepare(RVA_ScriptGame(0x447F3), &AI_HoldInventoryItems_Draw_Effect)
        .AddThisArg()
        .AddRegArg(mERegisterType_Esi)
        .Hook();
}
