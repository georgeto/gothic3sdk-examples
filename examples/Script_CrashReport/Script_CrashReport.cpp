...
{
  public:
    void PrintMessage(bCGothic3EngineDialogs::bEGE3DialogsLogMessageType a_MessageType, GELPCChar a_strMessage,
                      GEBool a_bFatal = GETrue)
    {
...
void PrintModules(bCErrorAdmin_Helper &a_ErrorAdmin)
{
    ...
    if (EnumProcessModules(Process, Modules, sizeof(Modules), &BytesNeeded))
    {
        for (auto i = 0u; i < (BytesNeeded / sizeof(HMODULE)); i++)
        {
            ...
            a_ErrorAdmin.PrintMessage(
                bCGothic3EngineDialogs::bEGE3DialogsLogMessageType_ErrorCallstack,
                bCString::GetFormattedString("%08X-%08X %s", ModuleInfo.lpBaseOfDll,
                                             reinterpret_cast<DWORD>(ModuleInfo.lpBaseOfDll) + ModuleInfo.SizeOfImage,
                                             ModuleName));
        }
    }
    else
...
void bCErrorAdmin_PrintStackFrame(bSException_Pointers *a_pExceptionPointers)
{
    ...
    ErrorAdmin.PrintMessage(bCGothic3EngineDialogs::bEGE3DialogsLogMessageType_ErrorCallstack,
                            GetFirstStackTraceString(0xF, &StackTraceExceptionPointers));
    ...
    ErrorAdmin.PrintMessage(bCGothic3EngineDialogs::bEGE3DialogsLogMessageType_ErrorCallstack,
                            "------------------------------");
    ErrorAdmin.PrintMessage(bCGothic3EngineDialogs::bEGE3DialogsLogMessageType_ErrorCallstack,
                            GetRegisterString(a_pExceptionPointers));
...
{
    GE_MESSAGE_WARN(bCString::GetFormattedString(
        "  %s at point %d (%s)", This->GetEntity()->GetID().GetText().GetText(), a_iIllegalPointOffset / 12,
        VectorToMarvinString(This->GetPoint()[a_iIllegalPointOffset / 12]).GetText()));
}
} // namespace
...
void ApplyHooks()
{
    ...
    Hook_SetUnhandledExceptionFilter.Hook(GetProcAddress("kernel32.dll", "SetUnhandledExceptionFilter"),
                                          &NopSetUnhandledExceptionFilter);

    // Print modules and stack dump
    static mCFunctionHook Hook_bCErrorAdmin_PrintStackFrame;
    Hook_bCErrorAdmin_PrintStackFrame.Hook(
        PROC_SharedBase("?PrintStackFrame@bCErrorAdmin@@IAEXPAUbSException_Pointers@@@Z"),
        &bCErrorAdmin_PrintStackFrame);
    ...
    Hook_bCErrorAdmin_CallFatalError_PrintStackFrame
        ...
        .AddRegArg(mERegisterType_Esi) // bCErrorAdmin
        .InsertCall()
        .Hook();
    ...
    Hook_NavZone_PrintIllegal.Prepare(RVA_Game(0x11465C), &NavZone_PrintIllegal)
        .AddRegArg(mERegisterType_Esi) // NavZone
        .AddRegArg(mERegisterType_Ebp) // IllegalPointOffset
        .InsertCall()
        .Hook();
}
